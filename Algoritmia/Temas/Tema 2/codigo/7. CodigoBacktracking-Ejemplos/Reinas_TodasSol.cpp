...
void reinaBT(int *reinas, int fila,int n_reinas);
    ...
    do{
       ...
       if(valido(reinas,fila)){       // Comprobar si el tablero es válido                  
                  if(fila==n_reinas-1){    // Todas las reinas colocadas
                       ...
                       reinaBT(reinas,fila+1,n_reinas);
                  }
       }
    }while(reinas[fila]!=(n_reinas-1));  // Hasta que recorran todas las columnas
}
